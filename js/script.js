	...
	request.onreadystatechange = function() 
	{
		...
		for (var key of mapReviewBaseline.keys()) 
		{
			if(key === url)
			{
				...
				document.getElementById('explanationbaseline').innerHTML ='Ti suggerisco il locale '+ nome_locale + ' perchè: "' + explanationBaseline + '"';
			}
		}
...
function letturaReviewUnique(contesto,url)
{
	...
	request.onreadystatechange = function() 
	{
		...
		for (var [key, value] of mapReview1)  
		{
			if(key === url)
			{
				var explanationAggregation = 'E’ un locale adatto per ' + getStringContext(contesto) + ' per questo motivo: '+ value +'. ';
...
function setExplanation1(datiInput,urlConsigliato) {
	...
	switch (num_totali) 
	{
		...
		case 2:
			...
			{
				document.getElementById('explanation1').innerHTML = "non sono state rilevate spiegazioni per il contesto: "+datiInput[2];
			}
		...
		case 3:
			...
			{
				document.getElementById('explanation2').innerHTML = "non sono state rilevate spiegazioni per il contesto: "+datiInput[2];
			}
			else if(expl3 === '')
			{
				document.getElementById('explanation3').innerHTML = "non sono state rilevate spiegazioni per il contesto: "+datiInput[3];
			}
		...
		case 4:
			...
			{
				document.getElementById('explanation4').innerHTML = "non sono state rilevate spiegazioni per il contesto: "+datiInput[4];
			}
		...
		case 5:
			...
			{
				document.getElementById('explanation5').innerHTML = "non sono state rilevate spiegazioni per il contesto: "+datiInput[5];
			}
